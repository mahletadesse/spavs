...
check_input=function(Y, X,  h0, h, nu, sigma_0.square, rho,  alpha_0, beta_0, psplit, tempering, inv.temp, tchains, tswaps, mws, thin, cores){
 ...
 if(tempering){ 
    if(length(inv.temp)==1){
         if(inv.temp<=0 || inv.temp>1/tchains) stop("'deltatemp' is a  non-negative parameter between [0, 1/tchains] ")   
        
         inversetemp= 1 -(0:(tchains-1) )*inv.temp           
    }
    else if(length(inv.temp)==tchains){
         if(any(inv.temp<=0) || any(inv.temp>1))  stop(" 'inv.temp' vector must have entries in (0,1] ") 
         if(!any(inv.temp==1))  stop("One entry of inv.temp must be 1 ")      
         if(length(unique(inv.temp))!=tchains) stop("Entries of 'inv.temp' must  be different from each other ") 
